...
{
public:
	virtual int listening(uint16_t port) = 0;
...
private:
	...
	virtual int deal_message(const connection, std::shared_ptr<loadbalance_respond_media_pull_message>);
	...
	virtual int deal_message(const connection, std::shared_ptr<respond_media_pull_message>);
