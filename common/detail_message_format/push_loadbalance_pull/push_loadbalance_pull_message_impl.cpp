...
namespace media_core_message
{
	...
	int push_loadbalance_pull_message_impl::full_data_direct(uint32_t tid, const std::vector<uint32_t>& sids, const std::vector<uint32_t>& ips, const std::vector<uint16_t>& ports)
	{
		_tid	= tid;
		_sids	= sids;
		_ips	= ips;
	...
	int push_loadbalance_pull_message_impl::give_me_data(uint32_t& tid, std::vector<uint32_t>& sids, std::vector<uint32_t>& ips, std::vector<uint16_t>& ports)
	{
		tid		= _tid;
		sids	= _sids;
		ips		= _ips;
