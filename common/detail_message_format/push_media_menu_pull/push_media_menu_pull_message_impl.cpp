...
{
	int push_media_pull_message_impl::send_data_to(int sockfd)
	{
		if (sockfd < 0)
		{
		...
		head_buffer[0] = MSGTYPE_PUSHMEDIAMENU;
		...
		uint32_t video_name_total_length = 0;
		for (unsigned int i = 0; i < _video_names.size(); ++i)
		{
			video_name_total_length += _video_names[i].size();
		}

		dzlog_info("total send size: %d", video_name_total_length);

		uint8_t* names_buffer = new uint8_t[video_name_total_length + 1];
		uint32_t accu_length = 0;
		for (unsigned int i = 0; i < _video_names.size(); ++i)
		{
			uint32_t this_time = _video_names[i].size();
			memcpy(names_buffer + accu_length, _video_names[i].c_str(), this_time);
			accu_length += this_time;
		}

		names_buffer[video_name_total_length] = 0;
		dzlog_info("send videos: %s", (char*)names_buffer);

		once = 0;
		for (unsigned int i = 0; i != video_name_total_length; i += once)
		{
			once = send(sockfd, names_buffer + i, video_name_total_length - i, 0);
		}
	...
	int push_media_pull_message_impl::full_data_remote(int sockfd, uint32_t tid, uint32_t length)
	{
		if (sockfd < 0)
		{
			dzlog_error("sockfd < 0");
	...
	int push_media_pull_message_impl::full_data_direct(uint32_t tid, const std::vector<std::string>& video_names)
	{
		if (!video_names.size())
		{
	...
	int push_media_pull_message_impl::give_me_data(uint32_t& tid, std::vector<std::string>& video_names)
	{
		tid = _tid;
	...
	void push_media_pull_message_impl::print_data()
	{
	...
	push_media_pull_message_impl::push_media_pull_message_impl()
	{
		clear();
	}
	...
	void push_media_pull_message_impl::init()
	{
		clear();
	}

	void push_media_pull_message_impl::clear()
	{
		_tid = _length = 0;
		_video_names.clear();
	}
